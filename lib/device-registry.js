...
function normalizeIngestBatch(value) {
  ...
  return {
    ...
    agentInstalledReported: Boolean(value.agent && typeof value.agent === "object" &&
      Object.prototype.hasOwnProperty.call(value.agent, "installed"))
  };
}
...
function publicDevice(device, now = Date.now()) {
  ...
  return {
    ...
    installConfirmedAt: device.installConfirmedAt || null,
...
class DeviceRegistry {
  ...
  async rotate(id) {
    return this.mutate(async () => {
      ...
      device.installConfirmedAt = null;
      await this.writeManifest(manifest);
      return { device: publicDevice(device), token };
    });
  }
  ...
  async ingest(token, value) {
    ...
    return this.mutate(async () => {
      ...
      if (batch.agentInstalledReported) {
        device.agentInstalled = batch.agent.installed;
        device.installConfirmedAt = batch.agent.installed ? device.lastSeenAt : null;
      }
