  ...
  async ingest(token, value) {
    ...
    return this.mutate(async () => {
      ...
      if (recent.has(batch.batchId)) {
        ...
        await this.writeManifest(manifest);
        return { accepted: 0, duplicates: batch.events.length, cursor: device.lastCursor || batch.cursor, replay: true, serverTime: now };
      }
      ...
      return { accepted, duplicates, cursor: device.lastCursor, replay: false, serverTime: device.lastSeenAt };
    });
  }
