...
function deduplicateLineageRecords(records, options = {}) {
  ...
  const outputRecords = prepared.map((record) => {
    let parent = null;
    let prefix = { count: 0, usage: emptyUsage() };
    ...
    if (record.explicitParentThreadId) {
      if (cyclic.has(record.threadId)) {
        ...
        if (!parent) {
          ...
          prefix = sharedPrefix(record.rawEvents, parent.rawEvents);
          if (prefix.count) {
            explicitMatches += 1;
    ...
    const events = record.rawEvents.slice(prefix.count);
    const usageSplit = sumEventUsage(events);
    const inheritedTokens = prefix.usage.total_tokens;
    const branchTokens = Math.max(0, record.controlTokens - inheritedTokens);
    return {
      ...record.source,
      ...
      inheritedTokens,
      branchTokens,
      sharedPrefixEvents: prefix.count,
      forkParentThreadId,
      forkMatchKind
    };
  });
  ...
  const audit = {
    ...
    explicitMatches,
    inferredMatches,
    matchedForks: explicitMatches + inferredMatches,
    missingParents,
    noSharedPrefix,
    cyclicParents,
