...
中心服务会自动把当前主机登记为内置设备。要接入其他电脑，只需在设置的“远端设备采集”中添加设备，选择 Windows 或 Linux / macOS，把生成的一条命令交给远程用户运行。命令会自动下载采集器、保存专用令牌、执行首次同步并配置当前用户的自启动；远端不需要克隆仓库或手动创建计划任务。需要 Node.js 22 或更高版本。

Monitor 顶栏的“?”会打开远程接入帮助页；也可直接访问 `/help`。帮助页以“三步接入、一条命令”为主流程，并提供状态检查、立即同步、卸载、特殊 Codex 目录、Linux linger、手动 SQLite 快照、隐私边界与折叠式排障说明。设备设置面板会自动等待首次同步，并显示采集器版本、系统、自启动状态和最近同步结果。

安装后的常用命令（Windows 将路径中的 `/` 换成 `\` 也可）：
...
同步失败会按 1、5、15 分钟自动重试。首次扫描全部 SQLite / rollout 历史，之后按本地文件偏移增量读取，默认每 15 分钟同步，每批最多 500 个事件。重新接入会生成新令牌并立即废止旧令牌；服务端始终只保存令牌摘要。

“消耗仪表盘”的设备占比与月份选择器联动：每个月分别按总 Token 计算设备占比，同时展示该月输入、缓存输入、输出、API 等价成本及成本占比、拆分覆盖率；切换月份不会混入其他月份或累计数据。

采集数据严格限定为设备 ID、采集器版本、操作系统与同步状态、不可逆线程哈希、事件哈希、时间、模型、Token 拆分与 SQLite 总量快照，不包含标题、目录、来源、提示词、回复、代码、工作路径或 Monitor 登录密码。线程哈希与累计 Token 状态共同生成设备无关的事件 ID，因此复制到多台设备的 rollout 只统计一次；同一线程在另一台设备继续产生的新累计状态仍归属新设备。采集令牌和游标保存在远端用户目录的 `~/.gpt-monitor/`，不会写入仓库。
